...
async def take_ss(video_file):
    ...
    cmd = f"""vcsi "{video_file}" -t -w 1340 -g 4x4 --timestamp-font assets/DejaVuSans.ttf --metadata-font assets/DejaVuSans-Bold.ttf --template tiensiteo/helper/ssgen_template.html --quality 100 --end-delay-percent 20 --metadata-font-size 30 -o {out_put_file_name} --timestamp-font-size 20"""
...
async def genss_link(msg, video_link, output_directory, min_duration, no_of_photos):
    ...
    if duration > min_duration:
        ...
        for looper in range(no_of_photos):
            ...
            try:
                await msg.edit(
                    f"📸 <b>Chụp ảnh màn hình:</b>\n<code>{looper+1} trên {no_of_photos} ảnh chụp màn hình được tạo ra..</code>"
                )
