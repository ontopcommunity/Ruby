...
from tiensiteo.core import pyro_cooldown
        ...
        async def decorator(client, message: pyrogram.types.Message):
            if is_disabled:
                return await message.reply_text(
                    "Xin lỗi, lệnh này đã bị chủ sở hữu vô hiệu hóa."
                )
            if not message.from_user and no_channel:
                return await message.reply_text(
                    "Tôi không thể xác định người dùng. Hãy sử dụng lệnh của tôi trong trò chuyện riêng tư."
                )
            ...
            if self_admin:
                ...
                if me.status not in (
                    ...
                    return await message.reply_text(
                        "Tôi phải là quản trị viên để thực hiện lệnh này"
                    )
            ...
            if pm_only and message.chat.type != pyrogram.enums.ChatType.PRIVATE:
                return await message.reply_text("Lệnh này chỉ có thể được sử dụng trong các PM.")
            try:
